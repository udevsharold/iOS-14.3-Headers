/*
* This header is generated by classdump-dyld 1.0
* on Tuesday, March 2, 2021 at 10:56:19 AM US Time
* Operating System: Version 14.3 (Build 18C66)
* Image Source: /System/Library/PrivateFrameworks/Home.framework/Home
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@class NSMutableDictionary;

@interface HFThreadCommissioner : NSObject {

	NSMutableDictionary* _activeCommissioningForSetupCode;

}

@property (nonatomic,readonly) NSMutableDictionary * activeCommissioningForSetupCode;              //@synthesize activeCommissioningForSetupCode=_activeCommissioningForSetupCode - In the implementation block
+(id)sharedCommissioner;
-(id)init;
-(NSMutableDictionary *)activeCommissioningForSetupCode;
-(id)commissionAccessoryWithSetupCode:(id)arg1 threadIdentifier:(id)arg2 home:(id)arg3 ;
-(void)cancelCommissioningForSetupCode:(id)arg1 ;
-(void)cancelAllCommissioning;
@end

